<template>
  <vue-cal :disable-views="['years', 'year', 'week', 'day']" class="" locale="en" active-view="month" hide-view-selector
    :on-event-click="onEventClick" today-button
    @cell-click="logEvents('cell-click', $event), $emit('date-selected', $event)" :events="events">
    ...
    <template #events-count="{ events }">
      <span class="vuecal__cell-events-count shl" v-if="customEventsCount(events, 'SHL')">
        {{ customEventsCount(events, 'SHL') }}
      </span>
      <span class="vuecal__cell-events-count chl" v-if="customEventsCount(events, 'CHL')">
        {{ customEventsCount(events, 'CHL') }}
      </span>
      <span class="vuecal__cell-events-count other" v-if="customEventsCount(events, 'other')">
        {{ customEventsCount(events, 'other') }}
      </span>
      <span class="vuecal__cell-events-count ha" v-if="customEventsCount(events, 'HA')">
